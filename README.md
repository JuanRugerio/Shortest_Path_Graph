...
- ##### TCP & Grizzly Servers:
  Listen for multiple requests from the UI. Communicate with the chosen pathfinding service (Grizzly run by Main) or algorithm (TCPServer). Sends the calculated path back to the UI.
...
The system offers five options to find the shortest path:

- #### Via a API,
  which receives the information about both points on the map (Via GET, POST, POST Geojson), for which the shortest path is to be known, and which returns the path.
